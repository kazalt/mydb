...
mysql.prototype.buildFunction = function(model) {
  var data = "";
  switch (model.queryOptions.type) {
    case "max":
      data = "MAX("+this.buildFieldKeyString(model.queryOptions.value)+")";
      break;
    case "min":
      data = "MIN("+this.buildFieldKeyString(model.queryOptions.value)+")";
      break;
    case "count":
      data = "COUNT("+this.buildFieldKeyString(model.queryOptions.value)+")";
      break;
    case "sum":
      data = "SUM("+this.buildFieldKeyString(model.queryOptions.value)+")";
      break;
    case "concat":
      var elements = [];
      var mthis = this;
      _.each(model.queryOptions.value, function(elem) {
        elements.push(mthis.buildFieldKeyString(elem));
      });
      data = "CONCAT(" + elements.join() + ")";
      break;
  }
  return data;
};
...
mysql.prototype.buildFieldKeyString = function(field) {
  var key = "";
  if (field instanceof functionModel) {
